...
- Launch templates user data: priority is `user_data_base64` > `user_data_file` > inline `user_data`; `user_data_file` supports repo-relative or absolute paths. `user_data_file`/inline `user_data` support bash-safe EKS cluster tokens such as `@@cluster.endpoint@@` and `@@cluster.certificate_authority@@`; set `user_data_cluster` when the cluster cannot be inferred from exactly one `vpc_security_groups`/`security_groups` cluster reference.
...
- EBS/EFS storage: use `ebs_volumes` for zonal block volumes and optional EC2 attachments; use `efs_file_systems.mount_targets` for subnet-level NFS endpoints and `efs_file_systems.access_points` for POSIX-scoped application paths.
- ACM certificates: `acm_certificates` supports `source=managed` (Terraform creates ACM certificates) and `source=existing` (reuse existing ACM certificates). Use optional `name` as a logical reference key (defaults to `domain_name`), and optional `region` (`us-east-1` for CloudFront certificates). For `source=existing`, either set `certificate_arn` directly or let Terraform look up by `domain_name` with `lookup_statuses`/`most_recent`. For managed certificates, `wait_for_issued` (with optional `validation_record_fqdns`) can wait until ACM reaches `ISSUED`.
- ALB listeners: `ec2_load_balancers.listeners` supports `default_action.type = fixed-response` with `fixed_response.content_type`, `fixed_response.status_code`, and optional `fixed_response.message_body`. HTTPS listeners support either explicit `certificate_arn` or `acm_certificate_name` (`acm_certificate_domain_name` alias) resolved from `acm_certificates[].name` (fallback: `domain_name`). If `default_action` is omitted, the listener defaults to forward action using the first item in `target_groups`.
